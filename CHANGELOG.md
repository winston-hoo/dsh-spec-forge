...
- **实测证据**：会话 `session-dd99e8c7` 中，需求「参考第三方对接文档页面…制作一个线下物料页面」的调用链是
  `spec_recall(57字) → spec_triage(622字) → spec_distill(1897字) → spec_retro(1009字)`，同一需求两轮共 8 次调用、7064 字。
  而用当前 `classifyComplexity` 回放这条需求，判定是 **L1 / fastTrack=true**（信号 `L1:self-contained` + `L1:has-reference`）。
...
### 量化效果（真实模板库 9 份模板，`<real-project-dir>` 为会话 cwd）
