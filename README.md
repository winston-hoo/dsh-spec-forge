...
```
收到需求 → spec_recall（翻历史模板、项目禁区，并给出 level / fastTrack / contentGap / nextStep）
                ↓
        按 nextStep 执行，不自作主张加步骤：
                ↓
   nextStep=confirm（contentGap 非空）──→ 用一次 ask_user_question 问清缺的内容，再动手
        需求只给了"容器"却没说是什么：加个按钮 / 加个路由 / 加一列 / 加个菜单项
                ↓ 否则
   nextStep=implement（fastTrack=true）──→ 直接动手，就这一步
        L1：取值型原子小改（改文案/调样式/改按钮样式/字段改名/改默认值）、
            单字段 CRUD、带参考物的自包含新建、或用户说了"不用问"
                ↓ 否则（nextStep=triage）
        spec_triage（四维体检 + 定级，不再重复扫模板库）
                ↓
   L2 按默认执行（不追问） / L3 完整 Grill-me
                ↓
        spec_distill（仅 L3 与安全阀场景）→ 动手写代码
                ↓
        spec_retro（任务链收尾、过了复用价值三问才沉淀）→ 下次同类需求被召回
```

> **0.4.3 起，`spec_recall` 的返回值里带一个 `nextStep` 指令**，常驻提示词段与 Skill 文档都改成
> 「严格按 `nextStep` 执行」。加这个字段是因为踩过一次坑：分级器明明判了 `fastTrack=true`，
> 模型却因为技能文档里「第 2 步：体检（**必做**）」的措辞，照样把六步走完——
> 一条一轮能做完的小需求白跑 3 次工具往返。**分级判得再准，也拦不住一个写着"必做"的流程。**
>
> **0.4.6 起 `nextStep` 从两态变三态**（`implement` / `confirm` / `triage`）。起因是用户质疑
> 「"登录页加个按钮"没写清加什么按钮、要做什么，你直接让它通过开始编写是不是有问题」——
> 结论是有问题，而且当时插件**明文写着「禁止追问」**，是模型自己留了 `// TODO: [待确认]` 才没出事。
> 判据：**缺「新增物的身份/用途」必须问，缺「已有物的属性取值」自己定。**
> 所以「加个按钮」现在会先问一次，而「改一下文案」「调一下间距」照旧一步直达。
...
在 v0.1.0 里，同样是这条需求，模型会一口气连问 5 个问题——哪怕每个问题它自己都能推断出答案。这就是 L1 通道要治的病：**该闭嘴干活时别装严谨**。0.4.0 扩了一类快速通道：**带参考物的自包含新建**——"参考现有列表页新建一个订单页"这类需求，实现细节可以从参考物自举，同样一轮做完、不问。**0.4.3 再补第三类：原子小改**——「改一下这个文案」「调一下间距」这类需求，此前全部落 L2、每次白跑一趟体检，现在同样一轮直达。

**0.4.6 又把这第三类拆成两半**（起因是一条用户质疑，见下方引用块）：「加个按钮」「加个路由」「加一列」虽然同样只是单点小改，但需求里没说清新增物**是什么、干什么**——替用户挑一个就是猜。这类改为 `nextStep=confirm`，先用一次 `ask_user_question` 问清再动手。判据一句话：**缺「新增物的身份」必问，缺「已有物的取值」自决。**
...
| 等级 | 什么算这类 | 插件怎么做 |
| --- | --- | --- |
| **L1 原子操作 / 快速通道** | 单文件 CRUD，字段/组件/默认值明确；带参考物的自包含新建；**原子小改（改文案/调样式/加按钮/加列/加搜索项/加路由/改默认值/字段改名）**；或消息含"直接做/速做/不用问/别问/不要问/极速模式" | 不许追问，直接给执行清单 + 扫描目标文件风格自举，疑虑标 `// TODO: [待确认]`；原子小改默认连沉淀都跳过 |
| **L2 模块变更** | 模块级新增/调整，一句话推不全 | **按报告默认值直接执行，不追问**；仅"过短 + ≥2 维缺失 + 零锚点"转一次性追问 |
| **L3 架构重构** | 含架构/重构/拆分/迁移/升级/建表/跨文件/多模块 | 完整 Grill-me，问透为止 |

原子小改的升级有三道闸门，宁可漏判也不误判：需求长度 ≤ 48 字、不含多任务连接词（以及/同时/顺便…）、不含大范围限定词（整体/全局/所有/批量…）。所以「加个按钮，同时把列表也重构一下」「统一所有按钮的文案」这类会被正确挡住，留在 L2。
...
- **词汇相似度是主项。** 指纹按 token 加权：路径(4) > 技术词(3) > 标识符(2) > 中文 2-gram(1)，"改 `index.vue`"比"有个页面"值钱得多。**0.4.5 起通用基名降权**：`index.vue`／`main.js`／`app.vue`／`package.json` 这类任何项目里都有一堆的基名不再吃满路径权重——否则 `…/login/index.vue` 与 `…/third-party-integration/index.vue` 会共享一个权重 4 的 `index.vue`，看起来像命中了同一条路径。
...
- **0.4.5 起，命中必须有词汇证据。** 调节项合计可达 0.30，而阈值只有 0.35——实测一份与需求**零词面重叠**的模板也能拿到 0.298（阈值的 86%）。现在 `lexical === 0` 一律不命中：先验分仍参与打分（用于排序），但"以前在同一个项目干过同类的事"不足以认定"这次该复用这份模板"。
...
| 无关但同仓库同分类（唯一交集是通用基名 `index.vue`） | 0.311 | ❌ 不命中（0.4.5 前是 0.372 误命中） |
...
| 项 | 量级 | 说明 |
| --- | --- | --- |
| 插件固定税 | ≈ 3.1K token / 请求 | 常驻提示段 ≈ 501 + 五个工具定义 ≈ 2545（只出现在发往模型的完整请求里） |
...
成本之外还有一笔**往返成本**：0.4.3 之前，简单需求也要走满 3 次工具往返。治理后按真实模板库回放 7 条样本，
合计调用从 22 次降到 14 次（−36%），其中简单需求全部从 3 次降到 1 次（−67%）——省的不只是 token，还有每一跳的固定税。
...
| 工具 | 什么时候被调用 | 干什么 |
| --- | --- | --- |
| `spec_recall` | 收到编程需求的**第一件事** | 翻模板库，把命中模板的澄清清单/标准改法/验收标准 + 项目禁区注入上下文，并给出 `level`、`fastTrack`、`contentGap` 与 `nextStep`（`implement` / `confirm` / `triage`）。**0.4.5 起判定 fastTrack 时省略「澄清清单」并滤掉改法里的 `spec_*` 流程行**——那两块分别与「禁止追问」「不要调 triage/distill」冲突，且占返回量七成。**0.4.6 起，需求只给了"容器"却没说是什么（加个按钮/加个路由/加一列）时返回 `nextStep=confirm`，要求先问清再动手** |
| `spec_triage` | `nextStep=triage` 时（即 `fastTrack=false`） | 四维体检 + 定 L1/L2/L3。L1 出执行清单，L2 出「已按默认执行」（仅安全阀转一次性追问），L3 出完整 Grill-me。0.4.3 起不再重复扫模板库，也不再需要 `cwd` 参数 |
...
| 层次 | 机制 | 内容 | 成本 |
| --- | --- | --- | --- |
| 常驻 | 系统提示词 section | 路由规则 + 等级响应 + 大文件纪律 + 急停规则，约 501 token（0.4.6 实测；超自设参考线 450 约 51 token，属有意保留，见 CHANGELOG） | 始终占用 |
...
```bash
npm test          # 单元测试：204 个，覆盖指纹（含通用基名降权）/匹配（含"先验分不得脱离词汇证据"）/会话提取/存储（含布局、空壳与禁区近重复去重）/渲染（含 fastTrack 注入裁剪）/分类器（含内容闸门与"路径不得当内容"反例）/路由契约（含提示段内容护栏）/沉淀门槛/读缓存/查询聚焦/路径解析/迁移
...
> 0.4.3 新增 `tests/routing.test.js`，把「快速通道必须真正短路」钉成回归测试：
> `nextStep` 必填、`fastTrack` 正文必须明文禁止调用 `spec_triage`/`spec_distill`、
> L2/L3 必须给出正向指令、`spec_triage` 不得再接收 `cwd`、召回未命中分支不得出现无条件沉淀指令。
>
> 0.4.4 追加一条**提示段内容护栏**：从 mock 的 `systemPrompt.section` 注册里抽出常驻段真文本，
> 断言 `order === 150` 且包含 16 条必需子串。起因是 0.4.3 压缩提示段时静默删掉了「报错排查」
> 等 4 条约束——**只比 token 数字看不出这种坏，必须有内容清单回归**。
...
4. 发一条带"直接做"的需求，观察是否无条件进 L1 快通道；发一条"参考现有列表页新建一个订单页"的需求，观察是否因**带参考物的自包含新建**直接 fastTrack；
   再发一条**取值型**原子小改（"改一下这个文案"/"调一下间距"），观察是否 `nextStep=implement`、**只调一次 `spec_recall` 就动手**（这是 0.4.3 的专项治理点）；
   最后发一条**容器型且没说是什么**的（"登录页加个按钮"），观察是否 `nextStep=confirm`、**先用一次 `ask_user_question` 问清再动手**（这是 0.4.6 的专项治理点）；
