...
| 工具 | 什么时候被调用 | 干什么 |
| --- | --- | --- |
| `spec_recall` | 收到编程需求的**第一件事** | 翻模板库，把命中模板的澄清清单/标准改法/验收标准 + 项目禁区注入上下文，并给出 `level`、`fastTrack` 与 `nextStep`（`implement` / `triage`） |
...
```bash
npm test          # 单元测试：182 个，覆盖指纹（含序列化回归）/匹配/会话提取/存储（含布局与空壳回归）/渲染/分类器/路由契约（含提示段内容护栏）/沉淀门槛/读缓存/查询聚焦/路径解析/迁移
