...
```
收到需求 → spec_recall（翻历史模板、项目禁区，并给出 level / fastTrack / nextStep）
                ↓
        按 nextStep 执行，不自作主张加步骤：
                ↓
   nextStep=implement（fastTrack=true）──→ 直接动手，就这一步
        L1：原子小改（改文案/调样式/加按钮/加列/加路由/改名）、
...
> **0.4.3 起，`spec_recall` 的返回值里带一个 `nextStep` 指令**（`'implement'` 或 `'triage'`），
> 常驻提示词段与 Skill 文档都改成「严格按 `nextStep` 执行」。
> 加这个字段是因为踩过一次坑：分级器明明判了 `fastTrack=true`，模型却因为技能文档里
> 「第 2 步：体检（**必做**）」的措辞，照样把六步走完——一条一轮能做完的小需求白跑 3 次工具往返。
...
在 v0.1.0 里，同样是这条需求，模型会一口气连问 5 个问题——哪怕每个问题它自己都能推断出答案。这就是 L1 通道要治的病：**该闭嘴干活时别装严谨**。0.4.0 扩了一类快速通道：**带参考物的自包含新建**——"参考现有列表页新建一个订单页"这类需求，实现细节可以从参考物自举，同样一轮做完、不问。**0.4.3 再补第三类：原子小改**——「登录页加个按钮」「改一下这个文案」「列表加一列」「加个路由」这类需求，此前全部落 L2、每次白跑一趟体检，现在同样一轮直达。
...
| 项 | 量级 | 说明 |
| --- | --- | --- |
| 插件固定税 | ≈ 2.8K token / 请求 | 常驻提示段 ≈ 426 + 五个工具定义 ≈ 2381（只出现在发往模型的完整请求里） |
...
| 工具 | 什么时候被调用 | 干什么 |
| --- | --- | --- |
| `spec_recall` | 收到编程需求的**第一件事** | 翻模板库，把命中模板的澄清清单/标准改法/验收标准 + 项目禁区注入上下文，并给出 `level`、`fastTrack` 与 `nextStep`（`implement` / `triage`）。**0.4.5 起判定 fastTrack 时省略「澄清清单」并滤掉改法里的 `spec_*` 流程行**——那两块分别与「禁止追问」「不要调 triage/distill」冲突，且占返回量七成 |
...
| 层次 | 机制 | 内容 | 成本 |
| --- | --- | --- | --- |
| 常驻 | 系统提示词 section | 路由规则 + 等级响应 + 大文件纪律 + 急停规则，约 426 token（0.4.4 实测） | 始终占用 |
...
```bash
npm test          # 单元测试：197 个，覆盖指纹（含通用基名降权）/匹配（含"先验分不得脱离词汇证据"）/会话提取/存储（含布局、空壳与禁区近重复去重）/渲染（含 fastTrack 注入裁剪）/分类器/路由契约（含提示段内容护栏）/沉淀门槛/读缓存/查询聚焦/路径解析/迁移
...
4. 发一条带"直接做"的需求，观察是否无条件进 L1 快通道；发一条"参考现有列表页新建一个订单页"的需求，观察是否因**带参考物的自包含新建**直接 fastTrack；
   再发一条原子小改（"登录页加个按钮"/"改一下这个文案"），观察是否 `nextStep=implement`、**只调一次 `spec_recall` 就动手**（这是 0.4.3 的专项治理点）；
