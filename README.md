...
```bash
# 单元测试：129 个用例，覆盖指纹/匹配/会话提取/存储/渲染/分类器/L1/L2/L3 报告/急停规则/沉淀门槛/标签推断
npm test
...
2. **匹配用的是加权关键词指纹，不是向量检索。** 好处是零依赖、零成本、可解释；
   ...
   打分构成为：`0.62×词汇相似度（0.6 加权余弦 + 0.4 覆盖率）+ 0.14×同分类 + 0.08×标签重叠
   + 0.08×同仓库 + 0.05×新鲜度 + 0.03×使用热度`。词汇相似度由「路径(4) / 技术词(3) /
   标识符(2) / 中文 2-gram(1)」四级权重的指纹算出；同分类按**一级**比较（`feature/api`
   与 `feature/pagination` 视为同类，因为二级名由模型自由填写、不可预测）；查询侧的
   tags / category 由 `inferQueryTags` / `inferQueryCategory` 从需求原文推断。
