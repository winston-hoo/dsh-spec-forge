...
import { downweightGenericBasenames } from './fingerprint.js'
 ...
 */
// ponytail: 打分 = 加权词面重合 + 时间/热度先验，没有向量检索，也没有学习排序。
// 上限：阈值 0.35 是经验值，对"又短又泛"的需求可能偏松；升级触发：出现假阳性召回样本
// （0.4.5 修的那次靠的是真实会话证据，不是调参直觉）。
export function scoreTemplate({ queryFp, template, repoHash, threshold = DEFAULT_THRESHOLD, now = Date.now() }) {
  // 两侧统一对通用基名降权（模板侧是历史指纹，旧版把它写成了满路径权重 4）
  const q = downweightGenericBasenames(queryFp)
  const t = downweightGenericBasenames(template.fingerprint ?? [])
  ...
  // 0.4.5：命中必须有词汇证据。
  //
  // 病因：调节项（同分类 0.14 + 同仓库 0.08 + 新鲜 0.05 + 热度 0.03）是**无条件**加总的，
  // 合计 0.30，而默认阈值只有 0.35 —— 实测一份与需求**零词面重叠**的模板也能拿 0.298
  // （阈值的 86%），词汇上再蒙对一个 token 就越线，产生假阳性注入。
  //
  // 判据：lexical === 0 意味着两侧没有任何共同 token，此时同分类/同仓库只能说明
  // "以前在同一个项目里干过同类的事"，不足以认定"这次该复用这份模板"。
  //
  // 为什么不设更高的 lexical 门槛：实测真命中的 lexical 跨度很大
  // （0.142~0.635 —— 中文描述对上以路径为主的指纹时天然偏低），
  // 抬高门槛会连真命中一起杀掉。所以只卡"零证据"这一条硬边界。
  const hasLexicalEvidence = lexical > 0

  return {
    score: round3(score),
    hit: score >= threshold && hasLexicalEvidence,
    breakdown: {
      cosine: round3(cos),
      coverage: round3(cov),
