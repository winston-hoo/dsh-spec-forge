...
//   ① 跳过词强制
//   ② 自包含新建 + 参考物（新页面/组件）
//   ③ 原子小改（改文案/调样式/加按钮/加列/加路由/改名），无字段名也能一轮做完
//
...
const L3_PATTERNS = [
  ...
  { name: 'ddl', pattern: /(建表|建库|建索引|加列|减列|修改表结构|新增表|删除表|新增索引|alter\s+table|create\s+table|drop\s+table|(?:新建|新增|创建|建|加)[\u4e00-\u9fa5A-Za-z0-9_\s]{0,15}(?<![列报图])表(?!单|格|头|尾|示|现|明|演|决|情|面|率|白|达|盘|针|带|记|稿|述|扬|层)|新增数据库)/i },
...
export const L1_ATOMIC_EDIT_PATTERNS = [
  ...
  // 按钮/图标类：「加个按钮」「换个图标」
  { family: 'button', pattern: /(?:加|新增|增加|添加|删|删除|去掉|移除|改|换|调整|修改)\s*(?:一个|个|这)?\s*(?:按钮|图标|icon)/i },
  { family: 'button', pattern: /(?:按钮|图标|icon)\s*(?:文案|样式|位置|名称|顺序|换行|大小)/i },
  ...
  // 路由/菜单类
  { family: 'route', pattern: /(?:加|新增|增加|添加|注册|删|删除|去掉|移除|改|调整|修改)\s*(?:一个|个|这)?\s*(?:路由|菜单项|菜单|导航项|子菜单)/ },
  // 默认值/常量类（动词前置）
...
export function classifyComplexity(requirement) {
  ...
  //    拿不到的原子小改才落到这里。三道闸门见 L1_ATOMIC_EDIT_PATTERNS 注释。
  if (text.length <= ATOMIC_EDIT_MAX_LEN && !L2_MULTITASK_SIGNALS.test(text) && !BROAD_SCOPE_SIGNALS.test(text)) {
    const hit = L1_ATOMIC_EDIT_PATTERNS.find((p) => p.pattern.test(text))
    if (hit) {
      return {
        level: 1,
        fastTrack: true,
        signals: ['L1:atomic-edit', `L1:atomic-edit:${hit.family}`],
        skipTrigger: null,
        inferredComponent,
        inferredField,
        inferredDefaultValue,
