...
/**
 * 通用基名：任何项目里都有一堆 index.vue / main.js。
 *
 * 为什么要单列（0.4.5）：路径基名在 tokenize 里拿的是**满路径权重 4**，
 * 于是一条完全无关的模板只要也含 `index.vue`，就和「…/login/index.vue」共享
 * 一个 w=4 的 token ——实测正是这一条把零相关模板顶到了 0.372（阈值 0.35）。
 *
 * 处理：保留该 token 但降权到最低档，而不是删除 ——「改 index.vue」这类真实
 * 需求不至于因此失去路径信号，只是不再靠它单独撑起词汇分。
 */
const GENERIC_BASENAMES = new Set([
  'index.vue', 'index.js', 'index.ts', 'index.tsx', 'index.jsx', 'index.mjs', 'index.cjs',
  'main.vue', 'main.js', 'main.ts', 'main.jsx', 'main.tsx',
  'app.vue', 'app.js', 'app.ts', 'app.jsx', 'app.tsx',
...
export function tokenize(input) {
  ...
  for (const p of rawPaths) {
    ...
    // 0.4.5：通用基名降权。它本身不携带任务信息，给满路径权重会让两个只共享
    // 「index.vue」的路径看起来像命中了同一条路径（见 GENERIC_BASENAMES 注释）。
    bump(seg[seg.length - 1], GENERIC_BASENAMES.has(seg[seg.length - 1]) ? WEIGHT_GRAM : WEIGHT_PATH)
 ...
 */
// ponytail: 词表 + 中文 bigram + 路径加权，**不接 embedding** —— 零依赖、可解释、离线可测；
// 召回错了的后果只是"少注入一份模板"，不是错改代码。上限：同义词与跨语言改写认不出
// （模板写"导出"、需求写"excel 下载"）；升级触发：模板库长到几十份后出现"该命中却没命中"的样本。
 ...
 * 对**已落盘的指纹**补做通用基名降权。
 *
 * 为什么需要：tokenize 侧的降权只影响"此刻新生成"的指纹，而库里的老模板是旧版写的，
 * 里面 `index.vue` 仍然是 4。只在查询侧降权的话，同一份修复对不同年代的模板效果不一致。
 * 打分前两侧统一过一遍，口径就与写入时间无关了（也不改动磁盘上的历史数据）。
 */
export function downweightGenericBasenames(fp) {
  const m = toWeightMap(fp)
  const out = new Map(m)
  for (const [token, weight] of m) {
    if (GENERIC_BASENAMES.has(token) && weight > WEIGHT_GRAM) out.set(token, WEIGHT_GRAM)
  }
  return out
}

/**
 * 查询侧聚焦：削掉长需求里的低信号尾巴，避免中文 2-gram 把余弦/覆盖率稀释。
 *
 * 为什么需要：真实用户需求往往很长（带路径、叙述、寒暄），fingerprint 默认取 24 个
 * token，其中一多半是权重 1 的中文 2-gram。这些 gram 大多不在模板指纹里，进了查询侧
 * 只会压低 coverage（分母变大）和 cosine（模长变大）。实测同样一条「管理页加 isMainAdmin
 * 开关字段」需求，原话 85 字与浓缩 25 字在同一模板上词汇分差 0.03~0.1，聚焦后差距消失。
 *
 * 规则：权重 ≥2 的强信号 token（路径 4 / 技术词 3 / 标识符 2）全部保留；
 * 权重 1 的 2-gram 最多留 maxGram 个（默认 8）兜底中文表述。
 * 只作用于查询侧、不碰落盘模板的指纹——模板指纹是"标准答案"，要保持完整。
 *
 * @param {Array<{token:string, weight:number}>} tokens fingerprint() 的输出
 * @param {number} [maxGram=8] 权重 1 的 2-gram 保留上限
 * @returns {Array<{token:string, weight:number}>}
 */
export function focusFingerprint(tokens, maxGram = 8) {
  const list = Array.isArray(tokens) ? tokens : []
  const strong = list.filter((t) => (t.weight ?? 1) >= 2)
  const grams = list.filter((t) => (t.weight ?? 1) < 2).slice(0, maxGram)
  return [...strong, ...grams]
}

/**
 * 从需求原文推断候选标签，用于与模板 tags 做重叠匹配。
 *
 * 形态刻意贴近沉淀时模型填写的 tags（小写英文、带连字符，如 vue / a-switch / table-column）：
 * 带连字符的复合词整体保留（否则 ant-design-vue 会被切碎成 ant/design/vue，与模板标签对不上）。
 * 打分侧会对标签做中英同义归一（match.js tagKey），这里按权重截断：
 * 数量太多会稀释单枚标签的权重排序，也降低归一后的覆盖率上限。
 *
