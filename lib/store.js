 ...
 * `youting-admin/dist 是 git 跟踪的目录，vite build 默认 emptyOutDir 会清空它；…`
 * 与 `验证构建必须显式 --outDir 到临时目录并事后清理；youting-admin/dist 是 git 跟踪目录…`
 * 归一化后并不相等，但共享绝大部分字面。
 *
 * ratio 默认 0.7 的依据（youting 真实档案 11 条禁区两两实测）：
 ...
 *
 * 0.4.5：旧版只用 `[...new Set(...)]` 做字符串级去重，实测 youting 项目档案里
