...
/**
 * 清理超过 days 天（默认 90）未使用的模板。
 * 作用域解析与 listTemplates 一致：scope 为仓库时合并项目层 + 全局层，删除发生在模板实际所在层。
 * 时间判定回退链 lastUsed → updated → created，三个字段都缺失的模板一律保留。
 * @param {string} home
 * @param {string} scope 当前仓库哈希
...
export function purgeStale(home, scope, days = 90) {
  ...
  let removed = 0
  for (const template of listTemplates(home, scope)) {
    const used = usedDateOf(template)
    if (!used || used >= cutoff) continue
    try {
