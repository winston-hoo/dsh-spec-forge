...
/**
 * 视为「真实改码」的工具名集合。
 * 只读诊断（read/grep/glob/ls/搜索类）与纯问答不计入。
 * 集合保持保守：宁可漏判（多提醒一次）也不误判只读为改码。
 */
 ...
 * 从会话事件中提取全部结构化事实。
 */
// ponytail: 复盘事实靠事件流模式匹配（工具名/路径/编辑次数），而不是把会话丢给模型读 ——
// 零 token、可单测、同一会话永远得到同一份摘要。上限：非标准工具（自定义 MCP）与
// 隐式代码改动识别不到；升级触发：出现真实"该沉淀却沉淀不出来"的会话。
export function extractSessionFacts(session) {
  ...
  const writeToolCalls = toolCalls.filter((c) => WRITE_TOOL_NAMES.has(c.name))
  ...
  return {
    ...
    writeToolCalls: writeToolCalls.length,
    codeChanged: writeToolCalls.length > 0,
    files,
    lastKind,
    todo: lastTodo,
...
/**
 * 沉淀门槛判定（第一层硬过滤，插件代码判定，不依赖模型自觉）。
 *
 * 只有当会话「真实动过代码」且工具调用达到下限时才认为有资格沉淀；
 * 纯问答、只读诊断、一次性脚本类任务在此被拦下。
 * 注意：门槛只判「有没有资格」，不判「值不值得」——后者由模型的复用价值三问负责。
 */
export function evaluateRetroEligibility(facts, options = {}) {
  const { minToolCalls = 2, requireCodeChange = true, sessionComplete = true } = options
  const toolCalls = facts?.toolCallCount ?? 0
  const writeToolCalls = facts?.writeToolCalls ?? 0
  // codeChanged 优先读字段；兼容无该字段的旧 facts（回退到 writeToolCalls）
  const codeChanged = facts?.codeChanged === undefined ? writeToolCalls > 0 : Boolean(facts.codeChanged)
  const reasons = []

  if (!sessionComplete) reasons.push('session-not-complete')
  if (toolCalls < minToolCalls) reasons.push(`tool-calls-below-${minToolCalls}`)
  if (requireCodeChange && !codeChanged) reasons.push('no-code-change')
