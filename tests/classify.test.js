...
test('classifyComplexity：单字段 + 组件 + 默认值 = Level 1（用户截图中的 case）', () => {
  const text =
    'index.vue 这个后台管理页面的新增/修改接口增加一个主管人员字段 isMainAdmin，值为1是，0否，默认为否，这个字段用开关来显示，请帮我完成这个需求'
  const r = classifyComplexity(text)
...
test('extractFieldName：识别多种字段名写法', () => {
  ...
  assert.equal(extractFieldName('增加一个主管人员字段 isMainAdmin'), 'isMainAdmin')
...
test('classifyComplexity：L1 单字段 CRUD 的 fastTrack=true', () => {
  const r = classifyComplexity('增加一个主管人员字段 isMainAdmin，开关，默认否')
  assert.equal(r.level, 1)
...
test('classifyComplexity：容器型但没说清"是什么" → 降 L2 并报 contentGap（0.4.6 闸门④）', () => {
  const cases = [
    ...
    ['@example-admin/src/views/login/index.vue 登录页加个按钮', 'button', ['按钮的文案与用途']],
...
test('classifyComplexity：带路径前缀不得被当成"内容已给"（0.4.6 实测踩过的坑）', () => {
  ...
  const withPath = classifyComplexity('@example-admin/src/views/login/index.vue 登录页加个按钮')
  ...
  assert.equal(withPath.inferredFile, 'example-admin/src/views/login/index.vue', '路径仍应被识别为目标文件')
})
