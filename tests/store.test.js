...
test('dedupeRedlines：括号说明不同但规则相同的条目会被合并（真实 youting 用例）', () => {
...
test('dedupeRedlines：改写了语序的近重复条目也会被合并（真实 youting 用例）', () => {
  const out = dedupeRedlines([
    'youting-admin/dist 是 git 跟踪的目录，vite build 默认 emptyOutDir 会清空它；验证构建必须显式 --outDir 到临时目录，事后清理；若已误写，用 git checkout HEAD -- youting-admin/dist 还原',
    '验证构建必须显式 --outDir 到临时目录并事后清理；youting-admin/dist 是 git 跟踪目录，被误写用 git checkout HEAD -- youting-admin/dist 还原',
  ])
  assert.equal(out.length, 1, '两条是同一规则的不同写法')
  assert.ok(out[0].startsWith('youting-admin/dist'), '应保留更完整的那条')
})

test('dedupeRedlines：不同规则不得被误合并', () => {
  const out = dedupeRedlines([
    'newShare/*.html 是设计稿基准，只读参考，任何情况下不改',
