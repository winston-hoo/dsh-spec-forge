...
test('spec_recall：容器型缺内容 => nextStep=confirm + contentGap 非空 + 明文要求先问（0.4.6）', () => {
  ...
  try {
    ...
    const req = '@youting-admin/src/views/login/index.vue 登录页加个按钮'
...
test('spec_recall：自包含新建 + 参考物（真实踩坑用例）=> nextStep=implement', () => {
  ...
  try {
    ...
    return recall
      .execute({
        requirement:
          '参考第三方对接文档页面，按照 newShare/parking-materials.html 这个页面内容制作，一个线下物料页面（放在 youting-admin 的 dynamic-routes 路由下）',
      })
      .then((res) => {
