...
**新装用户无须配置**。改了存储模式要搬数据？**数据就是一堆 markdown，直接拷目录即可** ——

> 0.6.0 删掉了 `spec_library({ action: 'info' | 'migrate' })`：15 个真实会话里它被调用 **0 次**，
> 却占 481 token/请求，而"看一眼路径、复制一个目录"本就是文件系统自带能力。搬库 = 把旧目录整个
> 拷到新位置（`global/` 与 `projects/<hash>/` 两层，都是普通 `.md`）。
...
0.6.0 起**不再有过期统计与清理动作**（真实库里只有 7 份模板；">90 天未命中就删"是纯推测需求，
能删它的 `spec_library` 也一并删了）。但"模板不是越多越好、旧模板会稀释召回精度"这条判断仍然成立 ——
想清理时直接删 `templates/` 下的 `.md` 文件即可：每份文件头部 frontmatter 里的 `name:` 就是模板名。
...
### 注释约定：`ponytail:` 标记（0.6.0）

代码里凡是**刻意选笨办法**的地方，用一行 `// ponytail:` 注释写明三件事：

1. 为什么这个简化是可接受的（错的代价有多大）；
2. **上限**（什么情况下它会不准/失效）；
3. **升级触发条件**（看到什么真实证据才值得换复杂方案）。

已标记 6 处：`lib/classify.js`（关键词判据）、`lib/fingerprint.js`（不接 embedding）、
`lib/match.js`（词面打分与 0.35 阈值）、`lib/extract.js`（事件流提取）、`index.js`（pre-step 幂等按原文哈希）、
`lib/store.js`（档案只写「禁区」一节）。

判据一句话：**没有"上限 + 升级触发"的简化注释，等于没写** —— 它只解释了过去，没告诉下一个人何时该推翻它。
